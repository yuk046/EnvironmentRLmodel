...
function [Environment]  = CreateEnvironmentMultiStep(environmentParameters)
%%define state space
...
for st = (environmentParameters.n_healthy_goals+1):(environmentParameters.n_healthy_goals+environmentParameters.n_base_states)
    ...
    for action=1:Num_Actions
        ...
        if(action<=environmentParameters.n_healthy_goals)           
            ...
            pid=id/environmentParameters.n_base_states;
            ps{st,action}=[pid*(1-environmentParameters.p_GetRewardGoals),(1-pid)*environmentParameters.p_GetRewardGoals];
            ps{st,action}=ps{st,action}/sum(ps{st,action});
